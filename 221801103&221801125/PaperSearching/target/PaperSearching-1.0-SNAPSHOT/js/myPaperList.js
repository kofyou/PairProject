$(function () {
  ...
  $.extend(Page.prototype, {
    init: function () {
      ...
      $.ajax({
        ...
        success: (data) => {
          ...
          if (maxPage == thisPage) $("#rightPage").css("cursor", "not-allowed");
          //发个ajax数据获取页面数据
          $.ajax({
              ...
              LIKE_LIST.forEach((like) => {
                $(".container").append(
                  "<div class='paper-item'>" +
                    ...
                    "</span> -->" +
                    "<span class='iconfont icon-shoucang shixin'></span>" +
                    "</div>" +
                    "<div class='paper-mid'>" +
                    "<span class='mid-word'>关键字：</span>" +
                    "<span class='paper-keyword'>" +
                    like.keyword.toString() +
                    "</span>" +
                    "</div>" +
    ...
    toPage: function () {
      ...
      } else window.open("myPaperList.html?page=" + p, "_self");
    },
    lastPage: function () {
      if (thisPage != "1") {
        window.open(
          "myPaperList.html?page=" + (parseInt(thisPage) - 1),
          "_self"
        );
      }
    },
